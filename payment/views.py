...
import requests
import json  # You'll need this for handling JSON data

# Import Some Paypal Stuff (we'll keep it for now but won't use it for the API call)
...
def billing_info(request):
    if request.POST:
        ...
        # Convert the 'totals' Decimal object to a string
        total_amount_str = str(totals)

        # Gather all data needed for the API
        data_to_send = {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('status') == 'success':
                    ...
                    # If user is authenticated, clear the cart in the database
                    if request.user.is_authenticated:
                        current_user = Profile.objects.filter(user__id=request.user.id)
                        current_user.update(old_cart="")
